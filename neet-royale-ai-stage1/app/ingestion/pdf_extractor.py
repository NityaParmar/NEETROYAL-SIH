"""
Stage 2: Extract raw text from every downloaded PDF, page by page using pypdf.

Run directly:
    python -m app.ingestion.pdf_extractor

For each page:
  1. Try pulling text straight from the PDF's embedded text layer (pypdf).
...
from pypdf import PdfReader  # Pure-python replacement for PyMuPDF
...
def extract_document(source_document_id: int, pdf_path: Path) -> dict[str, int]:
    ...
    for page_index, page in enumerate(reader.pages):
        page_number = page_index + 1  # 1-indexed for DB / logs
